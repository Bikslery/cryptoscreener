...
// adapter directly (source of truth). Use this right after a user spots an
// "empty place" to decide server-vs-wire/client.
...
router.get('/history-check', async (req, res) => {
  ...
  const exchange = (req.query.exchange as string | undefined) || undefined
  const mode = req.query.mode === 'exchange' ? 'exchange' : 'app'
  ...
  try {
    candles = mode === 'exchange'
      ? await fetchCandlesSeamless(symbol, tf, limit, exchange as Exchange)
      : await getHistory(symbol, tf, { limit, exchange: exchange as Exchange, before })
  } catch (e) {
